...
lamda=(300:10:900)*10^-9; %freespace wavelength of incident ray in meter 
thickness=2*10^-6;  %thickness of coating in meter 

f_v=0.2; %volume fraction. 0.01 corresponds to 1% 
...
r_vector=(1:200); %size range to be considered
weight_vector = lognpdf(r_vector,3.8,0.5); %the distribution
r_vector=(1:200)*10^-9;
...
for z=1:length(r_vector)
    ...
    for i=1:length(lamda)
        ...
        beta_sigma(i,z)=f_v*Qabs*Area/V +(1-f_v)*4*pi*k_medium(i)/lamda(i);
...
g=trapz(r_vector,(weight_vector.*g_arr_sigma.*alfa_sigma)')./trapz(r_vector,(weight_vector.*alfa_sigma)');
